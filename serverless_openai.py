...
DEFAULT_MODEL = "gpt-3.5-turbo"  # OpenAI model ID
    ...
    for config in payload_configs:
        try:
            ...
            generator = OutputGenerator(
                config, secrets, content_handler, history if keep_history else None
            )
...
class ContentHandler:
    ...
    def __init__(
        self, rossum: RossumPython, input_pairs: Dict[str, str], batch_size: int
    ):
        self.rossum = rossum
    ...
    def _generate_batched_input(self) -> List[Dict[str, Any]]:
        ...
        """
        single_values = {
            k: v for k, v in self._extracted_values.items() if not isinstance(v, list)
        }
        list_values = {
            k: v for k, v in self._extracted_values.items() if isinstance(v, list)
        }
    ...
    def _split_into_batches(self, user_input: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        total_items = max(
            (len(v) for v in user_input.values() if isinstance(v, list)), default=1
        )
        batches = []
...
class OutputGenerator:
    """
    Generates the output by interacting with the OpenAI API using the specified model.
    ...
    def _generate_single_output(self, user_input: Dict[str, Any]) -> str:
        """
        Generates a single output by sending the user input to the OpenAI API.
    ...
    def _send_request(self, messages: List[Dict[str, str]]) -> requests.Response:
        ...
        headers = {
            ...
            "Authorization": f"Bearer {self.secrets.key}",
        }

        response = requests.post(DEFAULT_API_URL, headers=headers, json=request_payload)
    ...
    def _extract_output_from_response(response_data: Dict) -> str:
        ...
        """
        if not response_data["choices"] or not response_data["choices"][0]["message"]:
